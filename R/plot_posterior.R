...
plot_posterior <- function(mcmc.sample, alpha=0.05, ncol=1){
  ...
  postplot <- ggplot(data=mcmc.sample, aes(x = sample, y = ..density..)) +
     ggtitle(paste("Posterior plots with ", formatC(100*(1-alpha), format="f", digits=0),"% credible intervals",sep=""))+
     geom_vline(data=qparm, aes(xintercept = quants)) +
     geom_density()+
     facet_wrap(~parm, ncol=ncol,scale="free")+xlab("Value of parameter")+
     geom_text(data=post_stat, aes(x=Inf, y=Inf, label=paste("Posterior mean: ",post.mean,"\nPosterior sd  : ",post.sd,sep=""),
                                   vjust=1, hjust=1))
  postplot
}
