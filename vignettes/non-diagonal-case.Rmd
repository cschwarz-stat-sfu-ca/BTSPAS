...
capture the final release strataum.
...
the next stratum, etc, upto a maximum number of recovery strata per release.
...
No recoveries occured until 4 days later.
...
to account for the number of releases plus potential movement, for a length of `r nrow(demo.data)+ ncol(demo.data)-4`:
```{r }
...
to give an estimate of `r formatC( round(Nhat$est), digits=0, width=20, format="f",big.mark=",")`.

Let us look at the data in more detail:

* In many of the release strata, there are small number of fish tagged and released.
* Most the recoveries occur soon afte release, but there is longer right tail.
...
Something funny appears to have happened around julian day 160 -- I suspect that some pooling of data has occured here.
...
# Fitting the BTSPAS non-diagonal model wth a log-normal movement distribution. 
...
demo.bad.m2     <- c()   # list julian weeks with bad m2 values. This is used in the Trinity Example
demo.bad.u2     <- c()   # list julian weeks with bad u2 values. [This was arbitrary to demostrate the feature.]
demo.bad.n1     <- c()   # list julian weeks with bad n1 values. [This was arbitrary to demonstrate the feature.]
...
Each release stratum is allowed to have a travel time distribution that differ from this base trvel time distribution,
by allowing the individual release stratum parameters to be sampled from a distibution around the above vales.
...
It is also possible to see the probabiity of moving from release stratum $i$ to recovery stratum $j$ by looking
...
# Fitting the BTSPAS non-diagonal model wth a non-parametric movement distribution. 
...
In both cases, the suspect values of $m2$ and $u2$ are set to missing.
Alternatively, the user can set the $m2$ and $u2$ values to missing in the data input direcly.
I arbitrarily chose the third julian week to demo2nstrate this feature.
...
demo2.bad.u2     <- c()   # list julian weeks with bad u2 values. [This was arbitrary to demo2strate the feature.]
demo2.bad.n1     <- c()   # list julian weeks with bad n1 values. [This was arbitrary to demo2nstrate the feature.]
...
# Prior information on the movement probabilties.
...
```
where the probability of movement in the statum of release and subsequent strata is
```{r}
...
demo3.bad.u2     <- c()   # list julian weeks with bad u2 values. [This was arbitrary to demo3strate the feature.]
demo3.bad.n1     <- c()   # list julian weeks with bad n1 values. [This was arbitrary to demo3nstrate the feature.]
