...
Recoveries occur from a commmerical fishery that only operating for 1/2 a week (the first half).
This can potentiall cause bias in estimates of abundance because, for example, fish tagged at the end of a week,
...
However, the actual implementation is incorrect in *BTSPAS* and is depricated. This vignette shows the
...
Returing salmon arrive and are captured at a fish wheel during several weeks. Those fish
captured at the fish wheel are tagged and released (daily). 
They migrate upstream to a commercial fishery. The commerical fishery does not operate
...
the commercial fishery. Consequently, we simulate the probability of capture that varies around 0.05. The distribution of capture probilities at the wheel is:
...
```
The distribution of catchability in the commerical fishery is
...
If the catchability in the commerical fishery equal for all fish, then the pooled Petersen will also be unbiased.
This is clearly not the case because some fish has a probabiity of 0 of being captured when the fishery is not operating.


If the probability of capture in the commerical fishery is uncorrelated with the probability of capture by the tagging wheel,
the pooled-Petersen is also unbiased. A plot of the probability of capture at the tagging wheels and in the commerical fishery is:
```{r  fig.width=6, fig.height=4,include=TRUE, fig.align="center", fig.cap=c("Distribution of cature-probabilities at wheel"), echo=FALSE, warning=FALSE, message=FALSE}
correlaton.plot
```
...
Notice that some columns that are all zero because of the commerical fishery.
...
ppz.complete$est <- ppz.complete$est + sum(takuz[,"tagged"],na.rm=TRUE) + sum(takuz["untagged",],na.rm=TRUE)
```
The pooled Petersen estimator of abundance is 
`r formatC(round(ppz.complete$est,0), digits=0, big.mark=',', format="f")` 
(SE
`r formatC(round(ppz.complete$se,0), digits=0, big.mark=',', format="f")` 
...
```{r zeroset}
# are there any days where the capture probability is fixed in advance?, i.e. because no commerical fishery
...
file.rename("data.txt",       paste(takuz.prefix,".data.txt",sep=""))
file.rename("CODAindex.txt",  paste(takuz.prefix,".CODAindex.txt",sep=""))
file.rename("CODAchain1.txt", paste(takuz.prefix,".CODAchain1.txt",sep=""))
file.rename("CODAchain2.txt", paste(takuz.prefix,".CODAchain2.txt",sep=""))
file.rename("CODAchain3.txt", paste(takuz.prefix,".CODAchain3.txt",sep=""))
file.rename("inits1.txt",     paste(takuz.prefix,".inits1.txt",sep=""))
file.rename("inits2.txt",     paste(takuz.prefix,".inits2.txt",sep=""))
file.rename("inits3.txt",     paste(takuz.prefix,".inits3.txt",sep=""))
```
...
We simulate the coarser stratifcation by taking the previous simulated population and pool adjacent 3-day strata.
The pooled data is:
...
ppzp.complete$est <- ppzp.complete$est + sum(takuzp[,"tagged"],na.rm=TRUE) + sum(takuzp["untagged",],na.rm=TRUE)
```
...
`r formatC(round(ppzp.complete$est,0), digits=0, big.mark=',', format="f")` 
(SE
`r formatC(round(ppzp.complete$se,0), digits=0, big.mark=',', format="f")` 
).
...
file.rename("data.txt",       paste(takuzp.prefix,".data.txt",sep=""))
file.rename("CODAindex.txt",  paste(takuzp.prefix,".CODAindex.txt",sep=""))
file.rename("CODAchain1.txt", paste(takuzp.prefix,".CODAchain1.txt",sep=""))
file.rename("CODAchain2.txt", paste(takuzp.prefix,".CODAchain2.txt",sep=""))
file.rename("CODAchain3.txt", paste(takuzp.prefix,".CODAchain3.txt",sep=""))
file.rename("inits1.txt",     paste(takuzp.prefix,".inits1.txt",sep=""))
file.rename("inits2.txt",     paste(takuzp.prefix,".inits2.txt",sep=""))
file.rename("inits3.txt",     paste(takuzp.prefix,".inits3.txt",sep=""))
```
