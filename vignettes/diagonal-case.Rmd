...
vignette: >
  %\VignetteIndexEntry{BTSPAS Diagonal Case}
...
with a long history. The standad Lincoln-Petersen 
estimator is used to estimate abundance as
...
account for heterogenous capture-probabilities is through stratification. For example,
...
is found by summing the two estimated abundances (one for eash sex).
...
kilometers down stream. At the second fishwheel, the captures consist of a mixtue of marked
...
capture-probabilities are (mostly) homogeneous at both wheels. Furthermore, suppsoe that
...
Marked fish from one stratum tend to move a similar rates and so are recaptured
...
Nhat$est <- Nhat$est + sum(demo.data$n1) + sum(demo.data$u2)
```
## Preliminary screening of the data

A pooled-Petersen estimator would add all of the marked, recaptured and unmarked fish
to give an estimate of `r formatC( round(Nhat$est), digits=0, width=20, format="f",big.mark=",")` which seems unbelievable!
...
estimated recapture probabiiity will be zero. But the data shows that some unmarked
...
* In some julian weeks, the number of marked fish released and recaptured is
...
There are two julian weeks where the number of umarked fish captured suddenly jumps by
...
computing a Petersen estimator for the total number of unmarked fishfor each individual stratum:
...
demo.data$Uest <- BTSPAS::SimplePetersen( demo.data$n1, demo.data$m2, demo.data$u2)$est
...
The $BTSPAS$ package attemps to strike a balance between the completely pooled Petersen estimator and the comletely stratified Petersen estimator.
...
in julian weeks 23 and 40, hatchery  resulting in sudden jump
...
* sometimes bad thing happend.  The vector $bad.m2$ indicates which julian weeks something went wrong. In the above example, the
number of recoveries in julian week 41 is far below expectations and leads to impossible
Petersen estimate for julian week 41. Similrly, the vector $bad.u2$ indicates which julian weeks, the number of unmarked fish is suspect.
In both cases, the suspect values of $m2$ and $u2$ are set to missing.
Alternatively, the user can set the $m2$ and $u2$ values to missing in the data input direcly.
I arbitrarily chose the third julian week to demonstrate this feature.

The $BTPAS$ function also allows you specifity
...
the pooled- and stratified-Petersen estiamtes, a test for pooling, and summaries of the posterior. Only the first few lines
are shown below:
```{r }
...
The stratums where the value of *p* is to be fixed is specified along with the value (on the logit scale) at which the capture probabilities are fixed.
...
```
Notice how the fixed values at $-10$ (on the logit scale0 occur.
...
## Reading in the data
Here is an example of some raw data that incudes the covariate $log(flow)$: 
```{r}
...
A prelimary plot of the empirical logit (exlcuding those weeks when the trap was not running) shows an approximate quadratic fit to $log(flow)$,
but the uncertainty in each week is enormous!
```{r  echo=FALSE, fig.height=4, fig.width=6, fig.caption="Logit(p) vs log(flow)"}
...
We need to create a matrix with the covariate values. We will need three columns - one for the intercept, the value of log(flow) and the square of it's values.
In practise, it is often advisable to standardize covariates to prevent numerical difficulties, but in this case, the values are small enough that standardization
...
Here is a plot of the estimated $logit(p)$'s vs. the log(flow):
...
```{r }
demo3.fit$summary[demo3.coeff.row.index,]
...
can be imposed by using a spline as the covariates for the $p$'s.
...
A prelimary plot of the empirical logit (exlcuding those weeks when the trap was not running) shows some temporal structure:
